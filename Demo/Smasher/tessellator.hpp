...
typedef struct __attribute__ ((packed)) CVertexTex {
    float s0, t0;
} CVertexTex;

typedef struct __attribute__ ((packed)) CVertexColor {
    float r, g, b, a;
} CVertexColor;
...
public:
    ...
    void LinkUp(CObjectNode* pNode) {
        if (m_pLeft && m_pLeft->m_pUp) {
            m_pLeft->m_pUp->m_pRight = pNode;
            pNode->m_pLeft = m_pRight->m_pDown;
        }
        if (m_pRight && m_pRight->m_pUp) {
            m_pRight->m_pUp->m_pLeft = pNode;
            pNode->m_pRight = m_pRight->m_pUp;
        }
        m_pUp = pNode;
        pNode->m_pDown= this;
    };
    ...
    void LinkDown(CObjectNode* pNode) {
        if (m_pLeft && m_pLeft->m_pDown) {
            m_pLeft->m_pDown->m_pRight = pNode;
            pNode->m_pLeft = m_pLeft->m_pDown;
        }
        if (m_pRight && m_pRight->m_pDown) {
            m_pRight->m_pDown->m_pLeft = pNode;
            pNode->m_pRight = m_pRight->m_pDown;
        }
        m_pDown = pNode;
        pNode->m_pUp = this;
    }
    ...
    void LinkLeft(CObjectNode* pNode) {
        if (m_pUp && m_pUp->m_pLeft) {
            m_pUp->m_pLeft->m_pDown = pNode;
            pNode->m_pUp = m_pUp->m_pLeft;
        }
        if (m_pDown && m_pDown->m_pLeft) {
            m_pDown->m_pLeft->m_pUp = pNode;
            pNode->m_pDown = m_pDown->m_pLeft;
        }
        m_pLeft = pNode;
        pNode->m_pRight = this;
    }
    ...
    void LinkRight(CObjectNode* pNode) {
        if (m_pUp && m_pUp->m_pRight) {
            m_pUp->m_pRight->m_pDown = pNode;
            pNode->m_pUp = m_pUp->m_pRight;
        }
        if (m_pDown && m_pDown->m_pRight) {
            m_pDown->m_pRight->m_pUp = pNode;
            pNode->m_pDown = m_pDown->m_pRight;
        }
        m_pRight = pNode;
        pNode->m_pLeft = this;
    }
...
protected:
    ...
    void SInterpolate(CNodeData& cDest, const CNodeData& cE1, const CNodeData& cE2, const CNodeData& cE3, const CNodeData& cE4, float fT, int nWrap)
    {
        ...
        if (nWrap == 0)
            cDest.m_cTexel.fU = calc_cathmull(cE1.m_cTexel.fU, cE2.m_cTexel.fU, cE3.m_cTexel.fU, cE4.m_cTexel.fU, fT);
        else if (nWrap > 0)
            cDest.m_cTexel.fU = calc_cathmull(cE1.m_cTexel.fU, cE2.m_cTexel.fU, cE3.m_cTexel.fU + 1, cE4.m_cTexel.fU + 1, fT);
        else
            cDest.m_cTexel.fU = calc_cathmull(cE1.m_cTexel.fU - 1, cE2.m_cTexel.fU, cE3.m_cTexel.fU, cE4.m_cTexel.fU, fT);
    ...
    void glFace(CNodeData& cV1, CNodeData& cV2, CNodeData& cV3, float fAlpha, float fUShift, float fVShift);
    void glEnv(CNodeData& cV1, CNodeData& cV2, CNodeData& cV3, float fAlpha);

    void glFaceFlush();
    void glEnvFlush();

#ifdef GL_VERSION_ES_CM_1_1
    int m_triBufSize, m_triEnvBufSize;
    int m_triCurrentIndex, m_triEnvCurrentIndex;

    CVertex *m_Vertex, *m_EnvVertex;
    CVertexTex *m_Texture, *m_EnvTexture;
    CVertexColor *m_Color, *m_EnvColor;
#endif
};
